...
CO2.anim<-function(){
    ...
    for(i in unique(CO2$year)){
      ...
      plot.CO2<-qplot(y=average,
                          ...
                          size=I(3),
                          xlab="Time (year)",
                          ylab=expression("Global average CO"[2](ppm)),
                          colour=type)
      plot.CO2<-plot.CO2 + scale_colour_manual(values = c("Ice core"="blue",
                                                         "Air measurements"="black",
                                                         "Lower predictions"="orange",
                                                         "Higher predictions"="red"))
      plot.CO2<-plot.CO2 + opts(legend.position = "none")
...
dev.off()
setwd("~/Documents/PhD/Reports/Conferences/CSEE/CSEE 2010/global co2 concentration/movie images")
ani.options(ani.height = 540, ani.width = 960, interval = 0.05, nmax = length(unique(CO2$year)))
# saveMovie(CO2.anim(),
#          interval=0.1,
